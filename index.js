const audit = {
  ...
  exerciseH() {
    ...
    const result =
    restaurants.reduce((acc, el) => {
      if(!acc[el.cuisine]) {
        acc[el.cuisine] = [];
      }

      acc[el.cuisine].push(el.name)
      return acc
    }, {})
...
//taking in a string
//want back out which side wins
//need to calculate value for each letter in the string
